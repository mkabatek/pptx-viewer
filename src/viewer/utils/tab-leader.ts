 ...
 *
 * OOXML tab stops can specify a leader character that fills the gap
 * between the preceding text and the tab stop position. This module
 * provides the mapping from OOXML leader types to repeated characters
 * and CSS styles for rendering them.
 *
 * @module viewer/utils/tab-leader
 */
 ...
 *
 * The leader span is rendered as an inline-block element that fills
 * the tab gap width with repeated leader characters via CSS
 * `text-overflow` or character repetition.
 *
...
/**
 * Generate a string of repeated leader characters sufficient to fill
 * the given pixel width. Assumes ~0.6em per character at the given
 * font size.
 *
