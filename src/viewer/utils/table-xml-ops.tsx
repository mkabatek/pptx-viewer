import type { PptxElement, PptxTableData, XmlObject } from 'pptx-viewer-core';
...
export function updateCellTextInRawXml(
	...
	const graphicData = (newRawXml['a:graphic'] as XmlObject | undefined)?.['a:graphicData'] as
		| XmlObject
		| undefined;
	...
	const cells = ensureArrayValue(rows[rowIndex]['a:tc'] as XmlObject | XmlObject[] | undefined);
	...
	const existingRuns = firstParagraph
		? ensureArrayValue(firstParagraph['a:r'] as XmlObject | XmlObject[] | undefined)
		: [];
	const firstRunProps =
...
export function updateMergeAttrsInRawXml(
	...
	const xmlRows = ensureArrayValue(table['a:tr'] as XmlObject | XmlObject[] | undefined);

	for (let rIdx = 0; rIdx < Math.min(tableData.rows.length, xmlRows.length); rIdx++) {
		const dataRow = tableData.rows[rIdx];
		const xmlCells = ensureArrayValue(xmlRows[rIdx]['a:tc'] as XmlObject | XmlObject[] | undefined);
		...
		for (let cIdx = 0; cIdx < Math.min(dataRow.cells.length, xmlCells.length); cIdx++) {
			...
			if (cell.text !== undefined) {
				...
				const existingParagraphs = existingTxBody
					? ensureArrayValue(existingTxBody['a:p'] as XmlObject | XmlObject[] | undefined)
					: [];
				const firstParagraph = existingParagraphs.length > 0 ? existingParagraphs[0] : undefined;
				const existingRuns = firstParagraph
					? ensureArrayValue(firstParagraph['a:r'] as XmlObject | XmlObject[] | undefined)
...
export function rebuildTableStructureInRawXml(
	...
	// ── Rebuild a:tr ──
	const existingXmlRows = ensureArrayValue(table['a:tr'] as XmlObject | XmlObject[] | undefined);
	...
	const newXmlRows: XmlObject[] = tableData.rows.map((dataRow, ri) => {
		...
		const existingCells = existingRow
			? ensureArrayValue(existingRow['a:tc'] as XmlObject | XmlObject[] | undefined)
			: [];
