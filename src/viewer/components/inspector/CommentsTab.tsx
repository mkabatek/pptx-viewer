...
export function CommentsTab({
	...
	if (!activeSlide) {
		return <div className='text-xs text-muted-foreground'>Select a slide to view comments.</div>;
	}

	return (
		<div className='space-y-3 text-xs'>
			<div className='flex items-center justify-between'>
				<div className='text-xs uppercase tracking-wide text-muted-foreground'>Slide Comments</div>
				<div className='text-[11px] text-muted-foreground'>{activeSlide.comments?.length || 0}</div>
			</div>

			{(activeSlide.comments || []).length === 0 ? (
				<div className='text-xs text-muted-foreground'>No comments on this slide yet.</div>
			) : (
